...
def sexSize(gene,sex):  #info sur le sex, sa taille est dans la colonne 3
	...
	sexSizeEvents_path = '{}/data/{}/{}_sexSizeEvents.csv'.format(path_data,gene,gene)
	df_size=pd.read_csv(sexSizeEvents_path)
	df_size_gene=df_size.loc[df_size['gene']==gene].loc[df_size['sexon']==sex].to_numpy()
	return df_size_gene

def eventsDup_df(gene,path='{}/data/{}/{}_eventsDup_withCols.txt'.format(path_data,gene,gene)): #fonction qui return la portion dans eventsDup du gene targeté
...
def ases_df(gene):  #fait le dataframe de l'ases du gene targete où on isole les colonnes d'intérets (chemin can, alt, ...) voir 2eme variable
	...
	ases_gene_path= '{}/{}/thoraxe/ases_table.csv'.format(path_data,gene)
...
def sim_graph_initial(gene):
	#a pair can appear multiple times, select it only once for the creation of its edge
	tmp_G=eventsDup_df(gene).sort_values("rank").drop_duplicates(subset=['sexA', 'sexB'], keep='first')
	tmp_G=nx.from_edgelist(tmp_G.to_numpy()[:,1:3], create_using=nx.Graph)
	return tmp_G

def sim_graph_workframe(gene):
    
	df_gene=eventsDup_df(gene)
	G=nx.Graph()
	for pair in df_gene.to_numpy():
		...
		if G.has_edge(pair[1],pair[2]):
			...
			G.add_edges_from([(pair[1],pair[2],{'info':[[pair[3]],pair[5],pair[6],pair[-2],pair[-1]]})])
	return G

def sim_graph_initial_bis(gene):
	df_gene=eventsDup_df(gene)
	G_bis=nx.Graph()
	for pair in df_gene.to_numpy():
...
def event_to_pairs(G,gene): #G is sim_graph_workframe
	...
	for eve in dico:
		dico[eve].append(ases_df(gene)[eve-1])

	return dico

def get_alnCols(G,A,B,eveDups):
	tmp=eveDups.loc[(eveDups['sexA']==A)&(eveDups['sexB']==B)].to_numpy()
	if len(tmp)>0:
		tmp=tmp[0]
...
def codage(eve2pair,C,d_X): 
	...
	for evebis in eve2pair: #on boucle sur les évènements pour vérifier si l'extension est compatible avec tout les évènements
		...
		print('-----------------------------',C,eve2pair[evebis][-1][0].split('/'),eve2pair[evebis][-1][1].split('/'),status_ext,d_X[evebis],evebis)
...
def extensionbis(gene):
	...
	g=sim_graph_workframe(gene)         #creation du squelette du sim graph (squelette car aucun extension n'est faite)
	eve2pair=event_to_pairs(g,gene)  #dico des evenements aux paires
	eveDup=eventsDup_df(gene)
	...
	for pair in g.edges():  #je loop sur toutes les paires
		...
		#	continue
		extension_marge=extension_marge_pair(g,A,B,eveDup)  #je calcul si je peux etendre 
		...
		flag=False
		for ind in np.where(np.array(extension_marge)>0)[0]: #je considère l'extension pour toutes les marges non nulles
			marge=extension_marge[ind]
			print('MARGE',marge)
			for eve in g.edges[(A,B)]['info'][0]: #pour chaque paire on boucle sur ses évènements
				print(eve,pair)
				...
				if g.edges[(A,B)]['info'][p]=='can': #path_bool me dit si je regarde le chemin can ou alt
				...
				if X not in path: #si X n'est pas dans le chemin can ou alt, cad que c'est une ancre
					path=allPaths[gene][1].split('/') #je recupere le chemin canonique global
					if X not in path:
						continue
				...
				#C est le candidat pour faire l'extension (qui sera de la forme C/X ou X/C)
					...
					print('X',X)
					if len(path)>1 and ((direction=='Cter' and path.index(X)<len(path)-1) or (direction=='Nter' and path.index(X)>1)): 
						if direction=='Cter':
						...
						if C=='start' or C=='stop' or (C not in eve2pair[eve][-1][path_bool].split('/')) or C.split('_')[0]=='0' or path[-1]==C or path[0]==C: #check if C is not start or stop node of ESG
							break
						else:
							if C.split('_')[0]=='0': #check if C is not a node '0_blabla' which are not real sex
								...
								if '$' in C:
									...
									length_C=sexSize(gene,C)[0][3]
								if length_C<=5: #si le candidat est de taille inf a 5 je l'ajoute
									print('C IS A NODE, INF TO 5', g.has_node(C))
									...
									if isgood:
										...
										if direction=='Cter': #and (X==A or X==B) interet du ET ?
											update=X+'$.'+C
											path=path.replace('{}/{}'.format(X,C),update)
										elif direction=='Nter':
											update=C+'.'+X +'$'
											path=path.replace('{}/{}'.format(C,X),update)
									...
									colA_bis, colB_bis = get_alnCols(g,A,B,eveDup)
									print('ZZZZZZZZZZZBEFORE',A,B,colA_bis,colB_bis)
									if X==A: #dans la paire (A,B) la seed X est A
										if B!=C:
											if g.has_edge(C,B):
												colB=get_alnCols(g,C,B,eveDup)[1]
											else:
												break
											if (float(colB_bis[0])<=float(colB[0])<=float(colB_bis[1]) or float(colB_bis[0])<=float(colB[1])<=float(colB_bis[1])) or (float(colB[0])<=float(colB_bis[0])<=float(colB[1]) or float(colB[0])<=float(colB_bis[1])<=float(colB[1])):
												...
												for edge in g.edges(X):
													if direction=='Cter':
														temp=g.edges[edge]['info'][-1].split('-')
														print('ZZZZZZZZZZZ',temp,edge,X)
														temp[1]=str(int(temp[1])+length_C)
														print('ZZZZZZZZZZZOA',temp,edge,X)
														g.edges[edge]['info'][-1]='-'.join(temp)
													if direction=='Nter':
														temp=g.edges[edge]['info'][-1].split('-')
														print('ZZZZZZZZZZZ',temp)
														temp[0]=str(int(temp[0])-length_C)
														if int(temp[0])-length_C<0:
															temp[0]=str(1)
														print('ZZZZZZZZZZZOA',temp,edge,X)
														g.edges[edge]['info'][-1]='-'.join(temp)
												eve2pair[eve][-1][path_bool]=path
										...
										if A!=C:
											if g.has_edge(A,C):
												colA=get_alnCols(g,A,C,eveDup)[0]
											else:
												print('AAAAA')
												print('mem_nodes',mem_nodes)
												break
											if (float(colA_bis[0])<=float(colA[0])<=float(colA_bis[1]) or float(colA_bis[0])<=float(colA[1])<=float(colA_bis[1])) or (float(colA[0])<=float(colA_bis[0])<=float(colA[1]) or float(colA[0])<=float(colA_bis[1])<=float(colA[1])):
												...
												for edge in g.edges(X):
													if direction=='Cter':
														temp=g.edges[edge]['info'][-2].split('-')
														print('ZZZZZZZZZZZ',temp,edge,X)
														temp[1]=str(int(temp[1])+length_C)
														print('ZZZZZZZZZZZO',temp,edge,X)
														g.edges[edge]['info'][-1]='-'.join(temp)
													if direction=='Nter':
														temp=g.edges[edge]['info'][-2].split('-')
														print('ZZZZZZZZZZZ',temp,edge,X)
					...
					flag=True
					print('end while, g.hasnodeA/B', g.has_node(A), g.has_node(A))
					if not g.has_node(A):
						A=mem_nodes[A]
					if not g.has_node(B):
						B=mem_nodes[B]
...
eventsDup_path = '{}/data/{}/{}_eventsDup_withCols.txt'.format(path_data,gene,gene)
allpath_path='{}/data/{}/{}_canonical_path.txt'.format(path_data,gene,gene)
...
def writecsv_ASRU():
	with open ('{}/data/{}/{}_ASRUs_table.csv'.format(path_data,gene,gene), 'w') as f, open ('{}/data/{}/{}_instances_table.csv'.format(path_data,gene,gene), 'w') as g:
		csv_writer1 = csv.writer(f, delimiter=',')
		csv_writer2 = csv.writer(g, delimiter=',')

		csv_writer1.writerow(['gene','ASRU','Nbinstances','max','min','moy','median','std','eventsRank'])
		csv_writer2.writerow(['instance','size','NbSex','ASRU','gene'])

		G_sim_gene=extensionbis(gene)
		#G_original_gene=sim_graph_initial_bis(gene)
		...
		for ASRU in ConnectedComp_gene:
			...
			for inst in ASRU:
				...
				if '.' not in inst:
					...
					gene_csv_line2.append(sexSize(gene,inst)[0][3])
					gene_csv_line2.append(1)
				else:
					sizesum=0
					for seed in inst.split('.'):
						if '$' not in seed:
							sizesum+=sexSize(gene,seed)[0][3]
						else:
							sizesum+=sexSize(gene,seed.rstrip('$'))[0][3]
